...
public class TestAnnotationsConfig implements ImportAware {
  ...
  public Object handleException(Exception exception) {
    for (AnnotationAttributes annotationAttribute : this.annotationAttributes) {
      Class<? extends Throwable> exceptionClass = annotationAttribute.getClass("exception");
      if (exceptionClass.isInstance(exception) && NoHandlerFoundException.class.isInstance(exception)) {
      ...
      } else if (exceptionClass.isInstance(exception) && NullPointerException.class.isInstance(exception)) {
        System.out.println("NullPointerException");
        String basePackage = annotationAttribute.getString("basePackage");
        HttpStatus errorCode = annotationAttribute.getEnum("errorCode");
        if (basePackage.equals("")) {
          return new ResponseEntity<>(Error500RuntimeExceptionResponse.of(errorCode, exception),
              errorCode);
        } else {
          try {
            Object obj = Class.forName(basePackage).newInstance();
